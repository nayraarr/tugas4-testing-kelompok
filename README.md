...
| TC-CI-01 | Input `<script>alert('XSS')</script>` di field keterangan transfer | Script tidak dieksekusi, form menampilkan pesan error validasi "Input mengandung karakter yang tidak diizinkan" | Form menolak input dengan pesan validasi, tidak ada alert box yang muncul, data tidak tersimpan ke database | PASS | ![TC-CI-01](image-18.png) |
| TC-CI-02 | Input `<h1>Hacked</h1><img src=x onerror=alert(1)>` di field keterangan transfer | Tag HTML tidak dirender, form menampilkan pesan error validasi | Form menolak input dengan pesan validasi "Input mengandung karakter yang tidak diizinkan", tag HTML tidak dieksekusi | PASS | ![TC-CI-02](image-19.png) |
| TC-CI-03 | Input `{{7*7}}` di field keterangan transfer | Input ditampilkan sebagai teks literal `{{7*7}}`, tidak dievaluasi menjadi `49` | Form menerima input, keterangan ditampilkan sebagai teks literal `{{7*7}}` di riwayat transaksi, bukan `49` | PASS | ![TC-CI-03](image-19.png) |
