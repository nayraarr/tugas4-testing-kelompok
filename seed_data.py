...
print(f"Supervisor: {sup.username} / password123")
...
print(f"Teller: {teller1.username} / password123")
print(f"Teller: {teller2.username} / password123")
...
for username, first, last, email, saldo in nasabah_data:
    ...
    print(f"Nasabah: {username} / password123  →  Rekening: {rek.nomor_rekening}  Saldo: Rp {saldo:,}")
...
top2 = TopUp.objects.create(rekening=r1, nominal=Decimal('500000'),  metode='virtual', status='pending')

from banking.services import proses_topup
proses_topup(top1, teller1, disetujui=True, catatan='Tunai diterima')

print("\n✅ Seed data berhasil dibuat!")
print("\nCredentials:")
print("  supervisor1 / password123  →  Supervisor Bank")
print("  teller1     / password123  →  Teller")
print("  teller2     / password123  →  Teller")
print("  nasabah1    / password123  →  Nasabah (Dewi Lestari)")
print("  nasabah2    / password123  →  Nasabah (Rizky Pratama)")
print("  nasabah3    / password123  →  Nasabah (Maya Wulandari)")
print("  nasabah4    / password123  →  Nasabah (Fajar Hidayat)")
